...
namespace WpfApp1
{
    ...
    public class YoloDetector
    {
        ...
        public YoloDetector(string modelPath, string classesPath = "Assets/classes.json")
        {
            _session = new InferenceSession(modelPath);

            if (File.Exists(classesPath))
                _classNames = JsonConvert.DeserializeObject<List<string>>(File.ReadAllText(classesPath))!;
            else
                throw new FileNotFoundException("Не найден classes.json", classesPath);
        }

        public List<YoloDetection> DetectObjectsWithBoxes(Bitmap image, float confThreshold = 0.25f)
        {
            var input = Preprocess(image);
            using var results = _session.Run(new[] { NamedOnnxValue.CreateFromTensor("images", input) });

            var output = results.First().AsTensor<float>();
            return ParseOutputWithBoxes(output, image.Width, image.Height, confThreshold);
        }

        // Препроцессинг входа (resize 640x640, нормализация)
        ...
        // Парсинг выхода (8 классов → 13 признаков)
        private List<YoloDetection> ParseOutputWithBoxes(Tensor<float> output, int origW, int origH, float confThreshold)
        {
            var detections = new List<YoloDetection>();

            int numPreds = output.Dimensions[1]; // 25200
            int numAttrs = output.Dimensions[2]; // 13 (x,y,w,h,obj + 8 классов)
            ...
            for (int i = 0; i < numPreds; i++)
            {
                ...
                {
                    float clsConf = output[0, i, 5 + c];
                    float finalConf = objConf * clsConf;
                    if (finalConf > maxClsConf)
                    {
                        maxClsConf = finalConf;
                        maxClsIdx = c;
                    }
                }

                if (maxClsIdx >= 0 && maxClsConf > confThreshold)
                {
                    int left = (int)((x - w / 2) / 640f * origW);
                    int top = (int)((y - h / 2) / 640f * origH);
                    int width = (int)(w / 640f * origW);
                    int height = (int)(h / 640f * origH);

                    detections.Add(new YoloDetection
                    {
                        Label = _classNames[maxClsIdx],
                        Bounds = new Rectangle(left, top, width, height),
                        Confidence = maxClsConf
                    });
                }
            }
